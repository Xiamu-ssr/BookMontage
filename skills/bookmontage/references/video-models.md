...
### Seedance 2.5 · 20 秒生产实账

2026-08-22，《真人秀水障碍竞赛》使用 2 张人物参考图、16:9、720p、有声，20 秒实付 `$4.632030`，ZenMux 计费量为 `432,900`。同一任务折合 `$0.231602/秒`；只按时长线性折算，10 秒约 `$2.316015`，30 秒约 `$6.948045`。这些折算仅用于同配置预算，不是套餐承诺。

## Seedance 2.5 的历史 30 秒实账
...
本次 20 秒 720p 实账线性外推到 30 秒为 `$6.948045`，与此前按像素面积得到的 `$7.56` 接近，说明“720p 明显便宜于 1080p”的方向得到实账支持；但两次剧情、参考图数量和任务复杂度不同，不能把差值全部解释为分辨率。
