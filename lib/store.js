...
export const tmpRoot = join(dataRoot, 'tmp');
export const cacheRoot = join(dataRoot, 'cache');
export const inspirationRoot = join(dataRoot, 'inspirations');
...
export function ensurePublicLinks() {
  ...
  const tmpLink = join(projectRoot, 'public', 'book-temp');
  const inspirationLink = join(projectRoot, 'public', 'inspiration-assets');
  for (const [link, target] of [[assetsLink, assetRoot], [tmpLink, tmpRoot], [inspirationLink, inspirationRoot]]) {
...
export function exportSnapshot() {
  const db = openStore();
  const snapshot = { format: 1, data_root: dataRoot, items: allItems(db), links: allLinks(db) };
  db.close();
...
export function verifyStore() {
  ...
  for (const item of items) {
    ...
    if (['asset','clip','film','temp_asset','inspiration_asset'].includes(item.type)) {
